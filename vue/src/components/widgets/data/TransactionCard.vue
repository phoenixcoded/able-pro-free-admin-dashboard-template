            ...
            <v-list-item class="py-4 px-6" v-for="(item, i) in transactionData" :key="i">
              <template v-slot:prepend>
                ...
                <v-avatar v-else size="40" :color="item.avatarColor" variant="outlined" rounded="md">
                  <span class="text-darkText text-headline-medium">
                    {{ item.avatarName }}
                  </span>
                </v-avatar>
              </template>
              <h6 class="text-body-large mb-0">{{ item.title }}</h6>
              <span class="text-label-small text-lightText">{{ item.subcontent }}</span>
              <template v-slot:append>
                <div class="text-right">
                  <h6 class="text-body-large mb-0">{{ item.price }}</h6>
                  <p :class="'text-body-medium mb-0 text-' + item.color">
                    <SvgSprite
                      :name="item.icon || ''"
